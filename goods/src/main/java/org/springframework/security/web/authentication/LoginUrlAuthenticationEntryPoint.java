 ...
 * <p>
 * Holds the location of the login form in the {@code loginFormUrl} property,
 * and uses that to build a redirect URL to the login page. Alternatively, an
 * absolute URL can be set in this property and that will be used exclusively.
 * <p>
 * When using a relative URL, you can set the {@code forceHttps} property to
 * true, to force the protocol used for the login form to be {@code HTTPS}, even
 * if the original intercepted request for a resource used the {@code HTTP}
 * protocol. When this happens, after a successful login (via HTTPS), the
 * original resource will still be accessed as HTTP, via the original request
 * URL. For the forced HTTPS feature to work, the {@link PortMapper} is
 * consulted to determine the HTTP:HTTPS pairs. The value of {@code forceHttps}
 ...
 * @since 3.0
 */
public class LoginUrlAuthenticationEntryPoint implements
	...
	protected String buildRedirectUrlToLoginPage(HttpServletRequest request,
		...
		if (forceHttps && "http".equals(scheme)) {
			Integer httpsPort = portMapper.lookupHttpsPort(Integer
					.valueOf(serverPort));
